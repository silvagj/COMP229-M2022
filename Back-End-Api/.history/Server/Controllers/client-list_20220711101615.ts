...
export function DisplayClientListPage(req: express.Request, res: express.Response, next: express.NextFunction): void 
{
   ...
   res.json({success: true, msg: 'Client-List Displayed Successfully', clients: clientsCollection, displayName:  UserDisplayName(req)  })
    }).sort({FirstName:1});
}
...
export function ProcessDeletePage(req: express.Request, res: express.Response, next: express.NextFunction): void 
{
  ...
  Client.remove({_id: id}, function(err: CallbackError)
  {
    ...
    res.json({success: true, msg: 'Successfully Deleted Client', client: updatedClient});
  });
}